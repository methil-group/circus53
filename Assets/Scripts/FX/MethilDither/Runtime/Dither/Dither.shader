Shader "Hidden/VolFx/MethilDither"
{    
    SubShader
    {
        Cull Off
        ...
        Pass    // 0
        {
            ...
            #pragma multi_compile_local METHILDITHER NOISE
...
/*#if defined(_LUT_SIZE_X16)
	        ...
	        sampler2D _MethilDitherTex;
            
	        sampler2D _PaletteTex;
	        sampler2D _QuantTex;
	        sampler2D _MeasureTex;
            
            uniform float  _MethilDither;
            uniform float  _Weight;
            
            uniform float4 _MethilDitherMad;
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                float grade   = 1 - saturate(pow(1 - measure / _MethilDither, 4) + .001); // can be customized and remaped via curve (dither pattern sample)
                float noise   = grad_sample(frac(mad(i.texcoord, _MethilDitherMad.xy, _MethilDitherMad.zw)), grade, _MethilDitherTex).r;

                
                half4 result = lerp(lut_sample(uvw, _PaletteTex), lut_sample(uvw, _QuantTex), step(measure, noise * _MethilDither));
                result.a *= col.a;
...
#if NOISE
                float measure = lut_sample(uvw, _MeasureTex);
                float noise   = tex2D(_MethilDitherTex, frac(mad(i.texcoord, _MethilDitherMad.xy, _MethilDitherMad.zw))).r;
